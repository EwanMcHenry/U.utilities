...
#' Custom Map Plotter with ggplot2 and Optional Quantile-Based Color Limits and Plotly Integration
#'
#' This function creates a map visualization using `ggplot2` and supports either
#' a `viridis` color scale or a custom gradient color scale, defined by the user.
#' It allows for detailed customization of color breaks, labels, and plot titles.
#' The function also provides optional features to set color limits based on user-defined quantile probabilities of the `to.plot` variable and to convert the ggplot map into an interactive Plotly figure.
#'
...
#' @param col.limits Numeric vector or NULL. The limits for the color scale. Default is `c(0, max(variable))`. Can also be determined by quantiles if `quantile.probs` is provided.
...
#' @param fill.line_size Numeric. Size of the boundary line for the fill regions. Default is 0.05.
#' @param fill.line_colour Character. Color of the boundary line for the fill regions. Default is "grey90".
#' @param background.fill Character. Fill color for the background layer. Default is "grey90".
#' @param background.size Numeric. Size of the background boundary lines. Default is 0.05.
#' @param background.colour Character. Color of the background boundary lines. Default is "black".
#' @param n.breaks Numeric. Number of breaks for the color scale. Default is 5.
#' @param round_to Numeric. Number of decimal places to round the color breaks to. Default is 1.
#' @param just_pretty Logical. If `TRUE`, generates "pretty" breaks for the color scale.
#' @param manual_breaks Numeric vector. Optional manual breaks for the color scale.
#' @param quantile.probs Numeric vector. Optional quantile probabilities for determining color limits. Overrides `col.limits` if provided.
#' @param use.plotly Logical. If `TRUE`, converts the ggplot object into a plotly interactive plot. Default is `FALSE`.
#'
#' @return A `ggplot` object or a `plotly` interactive plot if `use.plotly = TRUE`.
#'
#' @examples
#' # Example with user-defined quantile limits and interactive plot
#'
#' @export
map.ploter <- function(to.plot = variable,
  ...
  # If quantile.probs is provided, calculate col.limits based on quantiles
  if (!is.null(quantile.probs)) {
    col.limits <- quantile(to.plot, probs = quantile.probs, na.rm = TRUE)
  }
  ...
  # Convert to Plotly if use.plotly is TRUE
  if (use.plotly) {
    map <- ggplotly(map, tooltip = "text", dynamicTicks = TRUE) %>%
