...
#'
#' @param to.plot `terra` SpatRaster or `sf` object. The data values to be plotted. Can be continuous or categorical.
...
#' @param background sf object. A secondary spatial data layer for plotting outlines such as surrounding landmass (default is the same as `fillground`).
...
#' @param custom_palette Character vector. Custom color palette for categorical data (if applicable).
#' @param custom_labels Character vector. Custom labels for the color scale (if applicable).
...
#' @param fill.line_size Numeric. Line size for filling the `fillground` object. Default is 0.05.
#' @param fill.line_colour Character. Line color for filling the `fillground` object. Default is "grey90".
#' @param background.fill Character. Background fill color. Default is "grey90".
#' @param background.size Numeric. Size of the background layer. Default is 0.05.
#' @param background.colour Character. Background color. Default is "black".
#' @param n.breaks Numeric. Number of breaks for the color scale. Default is 5.
#' @param round_to Numeric. The value to round to for breaks and labels. Default is 1.
#' @param just_pretty Logical. If `TRUE`, prettify breaks and labels. Default is `TRUE`.
#' @param manual_breaks Numeric vector. Option to provide manual scale breaks directly.
#'
#' @return A `ggplot` object.
#'
#' @examples
#' # Example with continuous data (raster)
#' map.ploter(to.plot = your_raster_object, fill.scale.title = "Continuous Variable", main.title = "Map of Continuous Data", fillground = your_fillground_sf_object)
#'
#' # Example with categorical data (sf)
#' map.ploter(to.plot = your_sf_object, fill.scale.title = "Habitat Type", main.title = "Land Cover Classification", fillground = your_fillground_sf_object, custom_palette = ceh.col.pallette, custom_labels = ceh.full.habtype)
#'
#' @export
map.ploter <- function(to.plot,
                       fill.scale.title,
                       main.title,
                       sub.title,
                       ...
                       pltly.text = NULL,
                       transformation = "identity",
                       col.limits = c(0, max(to.plot, na.rm = TRUE)),
                       use.viridis = TRUE,
                       custom_palette = NULL,
                       custom_labels = NULL,
                       low.col = "white",
  ...
  # Check if `to.plot` is a SpatRaster or sf object
  if (inherits(to.plot, "SpatRaster")) {
    # For continuous raster data
    if (is.null(custom_palette) || is.null(custom_labels)) {
      ...
      clr.labels = colour.lable(x = values(to.plot),
                                lims = col.limits,
                                n = n.breaks,
                                round_to = round_to,
                                just_pretty = just_pretty,
        ...
        if (use.viridis) {
          scale_fill_viridis_c(trans = transformation,
                               ...
                               guide = guide_colorbar(
                                 ...
                                 barwidth = unit(50, units = "mm"), draw.ulim = FALSE,
      ...
      # For categorical raster data
      plot_scale = scale_fill_manual(
        name = fill.scale.title,
        values = custom_palette,
        labels = custom_labels,
        guide = guide_legend(title.position = 'top', title.hjust = 0.5, label.hjust = 0.5)
      )
    }

    to.plot <- as.data.frame(to.plot, xy = TRUE)  # Convert SpatRaster to data frame for ggplot
  } else if (inherits(to.plot, "sf")) {
    # For sf object
    plot_scale = scale_fill_manual(
      name = fill.scale.title,
      values = custom_palette,
    ...
    stop("`to.plot` must be either a `SpatRaster` or an `sf` object.")
  }
